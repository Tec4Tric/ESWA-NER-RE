...
model = AutoModelForTokenClassification.from_pretrained("allenai/scibert_scivocab_uncased", num_labels=len(label_names))
model.to(device)
...
def objective(trial):
    ...
    # Define model
    config = AutoConfig.from_pretrained("allenai/scibert_scivocab_uncased", num_labels=len(label_map))
...
if __name__ == "__main__":
    ...
    # Train final model
    final_model = AutoModelForTokenClassification.from_pretrained("allenai/scibert_scivocab_uncased",
...
model.save_pretrained("./SciBert_Ner_model")
